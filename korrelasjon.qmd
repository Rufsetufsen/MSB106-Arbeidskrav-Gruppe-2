...
# Arbeidsledighet: standardiser variabelnavn til ledig (varierte i originalkoden)
ledig <- arbledighet %>%
  std_k(kommunenummer) %>%
  std_year(år) %>%
  transmute(kommunenummer, år, ledig = prosent_ledige_i_året)
...
# 2) Enebolig kvpris per kommune-år (unik) + nabo-gjennomsnitt
kvpris_ene <- bolig %>%
  mutate(
...
# 3) Koble til netto utpendling og beregn korrelasjon
