...
nav_aarny <- nav_aarny |>
  ...
  select(knr24, år, prosent_ledige_i_året, antall_ledige_i_året) |>
  rename(kommunenummer = knr24)
```
...
```{r}
#| echo: false
save(syss, file = "syss.rdata")
save(syssendr0024, file = "syssendr0024.rdata")
```
...
demoKart <- demografi_total |>
  left_join(bef, join_by(kommunenummer, år)) |>
  filter(år == "2024" & aldersgruppe == "20-29") |>
  mutate(andelaldgruppe = bef/beftot*100) |>
  select(kommunenummer, andelaldgruppe) |>
  left_join(Kart, join_by(kommunenummer))
arbledighetKart <- arbledighet |>
  filter(år == "2024") |>
  select(kommunenummer, prosent_ledige_i_året, prosent_arbledig) |>
  left_join(Kart, join_by(kommunenummer))
